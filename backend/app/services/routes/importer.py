...
async def import_osm_routes(db: Session) -> int:
    """Fetch all OSM running routes for A Coruña and store in DB."""
    imported = 0

    async with OverpassClient() as client:
        raw_elements = await client.get_running_routes()
        logger.info(f"OSM: fetched {len(raw_elements)} elements")
